...
if(isset($_POST['view'])){
	...
	while($row = mysqli_fetch_assoc($result)){ ?>
		<div class="form-group">
                                    ...
                                    <input type="text" name="name" id="name" class="form-control" value="<?php echo $row['name'] ?>" disabled>
                                </div>
                                <div class="form-group">
                                    <label class="text-danger">*Reason for Calling</label>
                                    <textarea name="reason_call" id="reason_call" value="<?php echo $row['reason_call'] ?>" class="form-control" disabled> <?php echo $row['reason_call'] ?></textarea>
                                </div>
                                <div class="row form-group">
                                    <div class="col col-md-6">
                                        <label>*Number of Caller</label>
                                        <input type="number" name="number_caller" id="number_caller" value="<?php echo $row['number_call'] ?>" class="form-control">
                                    </div>
                                    <div class="col col-md-6">
